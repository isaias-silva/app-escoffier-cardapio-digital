...
export function ChangePasswordForm() {
    ...
    const updatePassword = async () => {
        ...
        try {
            ...
            return setMessage({type:'success',text:res?.data.message||"dados atualizados"})

        } catch (err:any) {
            return setMessage({type:'error',text:err.response.data.message||"erro interno no servidor."})
    ...
    return <div>
        ...
        <button onClick={()=>updatePassword()} className="bg-orange-500 text-white font-bold p-2 rounded-lg my-2 transition-all duration-300 hover:bg-orange-300">Confirmar</button>
    </div>
